...
- positions held through the weekend (swing / meanrev / xsectmom) sit at the
  broker with **GTC protective stop orders** attached (as of the Jul 11 fix —
  earlier DAY-tif legs expired at each close); those stops can only execute
  when the market reopens Monday. The bot's own per-cycle stop checks run in
  parallel whenever the process is up.
